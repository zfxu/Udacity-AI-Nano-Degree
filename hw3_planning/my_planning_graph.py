# This exercise takes heavy advantage of aimacode and the class declaration for each kind of object.
# To start quickly, read through the design for each kind of object such as base graph node (PgNode),
# action node (PgNode_a), state node (PgNode_s) and the planning graph which consists of the above
# (PlanningGraph).
...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        """S-level Plwebanning Graph node constructor
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        nodes_a = []
        for action in self.all_actions:               # loop through actions and determine which actions to add
            node_a  = PgNode_a(action)                # get action node
            level_s = self.s_levels[level]            # store the previous level
            if node_a.prenodes.issubset(level_s):     # check if action node has precondition is subset
                nodes_a.append(node_a)
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)       # make sure action node is added into an state node
                    node_a.parents.add(node_s)        # make sure state node is added into an action node
        self.a_levels.append(nodes_a)                 # adds all action nodes all together
    ...
    def add_literal_level(self, level):
        ...
        nodes_s = set()                           # Create empty set in the level to store actions
        for node_a in self.a_levels[level-1]:
            for node_s in node_a.effnodes:        # looping through the set of possible child state nodes
                nodes_s.add(node_s)               # add the current state node to nodes_s
                node_s.parents.add(node_a)        # Build the planning graph with action and state alternatively
                node_a.children.add(node_s)       # Build the planning graph with action and state alternatively
        self.s_levels.append(nodes_s)             # Append state nodes to the current list
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (add1 & rem2) | (add2 & rem1)       # return true if one action negates the effect of another
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # return true if the effect of one action is the negation of a precondition of the other
        if a1_pos.intersection(a1_pre_neg)  != set() or a2_pos.intersection(a2_pre_neg) != set() or \
            a1_neg.intersection(a2_pre_pos) != set() or a2_neg.intersection(a1_pre_pos) != set():
            return True
        else:
            return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precond_a1 in node_s1.parents:
            for precond_a2 in node_s2.parents:
                if not precond_a1.is_mutex(precond_a2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        s_levels_set = self.s_levels

        # the outer for-loop goes through all the states (or state levels)
        for i, level in enumerate(s_levels_set):
            node_set = set()
            # the 1st double for loop finds the pair of matching goal
            for n in level:
                for g in goal_set:
                    if expr(n.symbol) == g:
                        node_set.add(n)

            # the 2nd double for loop makes sure no pair is mutex
            none_mutex = True
            for n1 in node_set:
                for n2 in node_set:
                    if n1.is_mutex(n2):
            ...
            # if no mutex is found, level sum is incremented by 1
            if none_mutex:
                level_sum += 1
